...
使用 JNI 调用本机库，然后访问堆外数据是可能的，但性能开销很少使其适用：
...
FFM API 中的不安全方法不会带来与 JNI 函数相同的风险，例如，它们不能更改 Java 对象中最终字段的值。
...
这里描述的工作可能会使后续工作能够提供一个工具，从给定本机库的头文件开始，以自动化方式生成与该库互操作所需的本机方法句柄。
